...
$sec9 = $doc.Content.Find
$null = $sec9.Execute("9. CONCLUSIONES")

$sec8 = $doc.Content.Find
$null = $sec8.Execute("8. EVALUA")

$r = $doc.Range($sec8.Parent.Start, $sec9.Parent.Start - 1)

for ($i = $doc.Tables.Count; $i -ge 1; $i--) {
    $tbl = $doc.Tables.Item($i)
    if ($tbl.Range.Start -ge $r.Start -and $tbl.Range.End -le $r.End) {
        $tbl.Delete()
    }
}

# Re-find after table deletion
$sec9 = $doc.Content.Find
$null = $sec9.Execute("9. CONCLUSIONES")
...
Write-Host "Reemplazado con exito!"
