...
try {
    ...
    for ($i = 1; $i -le $doc.Paragraphs.Count; $i++) {
        ...
        if ($text -match "^8\.\s+EVALUA") {
            $startIndex = $i
        }
        if ($startIndex -gt 0 -and $i -gt $startIndex -and $text -match "^9\.\s+CONCLU") {
            $endIndex = $i - 1
            break
        }
    }

    if ($startIndex -gt 0 -and $endIndex -gt 0) {
        $startRange = $doc.Paragraphs.Item($startIndex).Range.Start
        $endRange = $doc.Paragraphs.Item($endIndex).Range.End
...
"@
        ...
        $range.Text = $newText
        $doc.Save()
        Write-Host "Sección 8 actualizada exitosamente."
    } else {
        Write-Host "No se encontró el rango del Capítulo 8 (startIndex: $startIndex, endIndex: $endIndex)."
    }
} catch {
    Write-Host "Error: $_"
} finally {
